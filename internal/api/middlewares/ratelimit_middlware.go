...
func respondWithBlock(c *gin.Context, ip, route string, timeout int) {
	...
	logger.Warnf("IP blocked for route", []logger.ParamPair{{Key: "ip", Value: ip}, {Key: "route", Value: route}, {Key: "timeout", Value: timeout}})
}
...
func respondWithWarning(c *gin.Context, ip, route string, warnCount int) {
	...
	logger.Warnf("Too many requests for route", []logger.ParamPair{{Key: "ip", Value: ip}, {Key: "route", Value: route}, {Key: "warn_count", Value: warnCount}})
}
